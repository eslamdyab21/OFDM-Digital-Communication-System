...
for i = 1:14
  ...
  for j = 1:100
    ...
    % channel effect
    y_cp = conv(x_cp,h)+(No_vect(i)/2).*randn(1,(length(x_cp)+length(h)-1));
    %y_cp = conv(x_cp,h);
    % Remove cyclic prefix
    y = y_cp(L:length(x_ifft)+L-1);
    x_time_domain = ifft(fft(y)./fft([h zeros(1,length(y)-length(h))]));
    % FFT
    y = fft(x_time_domain);
    % Demodulation
...
for i = 1:14
  ...
  for j = 1:100
    ...
    x_cp = [x_ifft(length(x_ifft)-L+1+1:length(x_ifft)) x_ifft];
    % channel effect
    y_cp1 = conv(x_cp,h1)+(No_vect(i)/2).*randn(1,(length(x_cp)+length(h1)-1));
    y_cp2 = conv(x_cp,h2)+(No_vect(i)/2).*randn(1,(length(x_cp)+length(h2)-1));
    %y_cp = conv(x_cp,h);
    % Remove cyclic prefix
