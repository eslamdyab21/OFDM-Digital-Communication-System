  ...
  h = (1/sqrt(2*L))*(randn(1,L)+1i*randn(1,L));
   %h = 1;
      for j = 1:ofdm_symbols
        ...
        x_ifft = ifft(sqrt(fft_size).*x_quad,fft_size);
        %Cyclic prefix
        ...
        % Channel Decoding
        x_hat = Channeldecoding(x_demod);
        
        count_err = count_err + biterr(x,x_hat);
      end
  Pe_frame_avg = Pe_frame_avg + (count_err/(N_bits*ofdm_symbols));
...
for i = 1:length(Eb_No)
  ...
  for j = 1:ofdm_symbols
    ...
    x_quad = QPSK(x_encoded);
    % IFFT 
    x_ifft = sqrt(fft_size).*ifft(x_quad,fft_size);
    %Cyclic prefix
    x_cp = [x_ifft(length(x_ifft)-L+1+1:length(x_ifft)) x_ifft];
    % channel effect
